  ...
  return (
    <footer className="border-t border-border px-5 py-16">
      <div className="mx-auto flex max-w-6xl flex-col gap-10 md:flex-row md:justify-between">
        <div className="max-w-sm">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            Specialist Gastroenterologist & Hepatologist · Dubai
          </p>
        </div>
        <div className="grid grid-cols-2 gap-10 text-sm md:grid-cols-3">
          <FooterCol title="Practice" items={["About", "Expertise", "Programs", "Insights"]} />
          <FooterCol title="Contact" items={["WhatsApp", "Call clinic", "Email", "Directions"]} />
          <FooterCol title="Languages" items={["English", "فارسی", "العربية"]} />
        </div>
      </div>
      <div className="mx-auto mt-16 flex max-w-6xl flex-col items-start justify-between gap-4 border-t border-border pt-8 text-xs text-muted-foreground md:flex-row md:items-center">
        <p>© {new Date().getFullYear()} Dr. Katayoon Homayoon. All rights reserved.</p>
